...
class GameNode {
	...
	synchronized void set_winner(int winner) throws IllegalArgumentException{
	...
	synchronized void get_xml(CodeXML xml, int indent) throws NullPointerException{
		if( xml == null ){
			throw new NullPointerException("CodeXML(class) has no instance.\n");
		}
		if(type == TYPE_TEAM){
			xml.add_line(indent, "<team id=\""+ number +"\" />");
		}else{
			xml.add_line(indent, "<game id=\""+ number +"\">");
			game[0].get_xml(xml, indent+1);
			game[1].get_xml(xml, indent+1);
			xml.add_line(indent, "</game>");
		}
	}
